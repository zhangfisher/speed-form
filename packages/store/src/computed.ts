...
export interface ComputedRefDraftOptions {
  context?: StoreComputedScope;
  keyPath: string[];
  fullKeyPath: string[];
  parent: any;
  depends?: readonly any[];
  type: "context" | "scope";
}
/**
 ...
 * @returns
 */
function getComputedRefDraft(draft: any, computedCtx: ComputedRefDraftOptions, storeOptions: StoreOptions) {
  let rootDraft = draft;
  // 提供一个hook，允许修改计算函数的根上下文
  if (typeof storeOptions.onComputedContext == "function") {
    const newDrafe = storeOptions.onComputedContext.call(draft,draft,computedCtx);
    if (newDrafe !== undefined) {
      rootDraft = newDrafe;
    }
  }
  const { context, keyPath, fullKeyPath, depends } = computedCtx;
  try {
...
function createAsyncComputedMutate<Store extends StoreDefine<any>>( stateCtx: ISharedCtx<Store["state"]>,params: IOperateParams,storeOptions: StoreOptions) {
  ...
  stateCtx.mutate({
    // 声明依赖
    deps: (state: any) =>
      (depends || []).map((deps: any) =>
        getVal(state, Array.isArray(deps) ? deps : deps.split("."))
      ),
    fn: (draft, params) => {
