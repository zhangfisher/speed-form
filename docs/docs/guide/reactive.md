...
从上面的例子可看到，当更新`Context.age`时，所有的子组件不管是否有使用`Age`均会重新渲染，而这是这是不必要的，因为子组件并没有使用到`Context`的数据。
...
以上`fullName`就是一个计算属性，它的值是由`firstName`和`lastName`计算得来的,当`firstName`和`lastName`变。
